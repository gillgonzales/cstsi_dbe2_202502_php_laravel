...
import { CardImage, CardVideo } from '../../../components/Cards/card.styled';

const ModalEditProduto = () => {
    const { data, editProduto } = useContext(ProdutosContext)

    // const srcImage = item?.media?.length ? item.media[0] : imageUrl;
    ...
    const [srcImage, setSrcImage] = useState(editingProduto?.media?.length ? editingProduto.media[0] : imageUrl)
    const [srcVideo, setSrcVideo] = useState(() => srcImage?.toLowerCase().includes('youtub') ? srcImage.replace('https://www.youtube.com/watch?v=', 'https://www.youtube.com/embed/') : null)
    ...
    useEffect(() => {
        if (editingProduto) {
            ...
            setSrcImage(editingProduto?.media?.length ? editingProduto.media[0] : imageUrl)
            setSrcVideo(() => editingProduto?.media[0]?.toLowerCase().includes('youtub')
                ? editingProduto?.media[0]?.replace(
                ...
                <form action="" method="get" onSubmit={onSubmit}>
                    <InputFileImage>
                        <div>
                            {!srcVideo ?
                                <CardImage>
                                    <img src={srcImage} ref={inputImageRef} />
                                </CardImage>
                                : <CardVideo>
                                    <iframe
                                        src={srcVideo}
